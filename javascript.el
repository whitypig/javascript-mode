...
(defun js-proper-indentation (parse-status)
  ...
  (save-excursion
    ...
    (let ((ctrl-stmt-indent (js-ctrl-statement-indentation))
          ...
          (beginning-of-script-tag (js-beginning-of-script-tag-p)))
            ...
            ((nth 1 parse-status)
             ...
             (if (looking-at "[({[][ \t]*\\(/[/*]\\|$\\)")
               ...
               (unless same-indent-p
                 ...
                 (skip-chars-forward " \t"))
               (current-column)))
            ((and (not beginning-of-script-tag) continued-expr-p)
             (+ javascript-indent-level javascript-expr-indent-offset))
            (t 0)))))

(defun js-beginning-of-script-tag-p ()
  "Return t if the current point is at the next line of script-beginning tag.
That is to say, return t if the current point is on the following line of
